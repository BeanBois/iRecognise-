...
reward_to = LIF(shape=(1,exc_per_module),du=0.2,dv=0.2, vth=0.8,name = 'reward')
...
for i,ld in enumerate(learning_dense_layers_1):
    ld.a_out.connect(reward_to.a_in)
    reward_to.s_out.connect(excitatory_modules_1[i])
   
for i,ld in enumerate(learning_dense_layers_2):
    ld.a_out.connect(reward_to.a_in)
    reward_to.s_out.connect(excitatory_modules_2[i])
...
while True:
    ...
    try:
        ...
        reward = 1 if prediction == final_label else -1
        reward_to
